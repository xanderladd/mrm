...
class IBLDataset(NeuralDataset):
    ...
    def __init__(self, config_path: str = None, **kwargs):
        ...
        self.unit_quality_thresh = config.get("unit_quality_thresh",.3)
    ...
    def _save_to_cache(self):
        ...
        cache_data = {
            ...
            'cluster_brain_regions': self.cluster_brain_regions,
            'config_subset': {
                'eid': self.eid,
    ...
    def _load_from_cache(self):
        ...
        try:
            ...
            self.cluster_brain_regions = cache_data.get('cluster_brain_regions', [])
    ...
    def _load_raw_data(self):
        ...
        if self.brain_regions:
            channels = one.load_object(self.eid, 'channels', collection=probe_spec if probe_spec else None)
        ...
        self.cluster_brain_regions = []


        for cluster_id in good_cluster_ids:
            # Find this cluster in the original arrays
            cluster_mask = cluster_ids == cluster_id
            if cluster_mask.any():
                cluster_idx = np.where(cluster_mask)[0][0]
                brain_id = cluster_brain_ids[cluster_idx]
                
                if not np.isnan(brain_id):
                    region_info = brain_regions.get(int(brain_id))
                    if region_info:
                        self.cluster_brain_regions.append(region_info.acronym[0])
                    else:
                        self.cluster_brain_regions.append('unknown')
                else:
                    self.cluster_brain_regions.append('unknown')
            else:
                self.cluster_brain_regions.append('unknown')

        print(f"    Cluster brain regions: {len(set(self.cluster_brain_regions))} unique regions")
