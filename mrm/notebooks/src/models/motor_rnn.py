"""Multi-Region RNN for decision-motor task"""
import torch
import torch.nn as nn

class MultiRegionRNN(nn.Module):
    def __init__(self, evidence_hidden=64, motor_hidden=64, evidence_input=2, comm_dim=32):
        super().__init__()
        ...
        self.evidence_rnn = nn.LSTM(evidence_input, evidence_hidden, batch_first=True)
        self.evidence_output = nn.Linear(evidence_hidden, 1)
        
        # Communication: Region 1 → Region 2
        self.communication = nn.Sequential(
            nn.Linear(evidence_hidden, comm_dim),
            nn.ReLU(),
            nn.Linear(comm_dim, 16)
        )
        
        # Region 2: Motor Control (receives communication + position feedback)
        self.motor_rnn = nn.LSTM(16 + 2, motor_hidden, batch_first=True)
    ...
    def forward(self, evidence_input):
        ...
        motor_velocity = torch.zeros(batch_size, seq_len, 2, device=device)
        ...
        motor_hidden = (torch.zeros(1, batch_size, self.motor_hidden_size, device=device),
                       torch.zeros(1, batch_size, self.motor_hidden_size, device=device))
        ...
        return {
            ...
            'evidence_states': torch.stack(evidence_states, dim=1),
            'motor_states': torch.stack(motor_states, dim=1)
        }