...
def train_mr_gnode(model, config):
    """Train MR-GNODE model"""
    ...
    if 'source_model' in config['data_params'] and config['data_params']['source_model'] == 'motor_rnn':
        ...
        for epoch in tqdm(range(n_epochs), desc="Training MR-GNODE"):
            ...
            for i in range(0, n_trials, batch_size):
                ...
                # Use first 2 dims of evidence as input
                ...
                # Forward pass
                outputs = model(batch_input)  # [batch, time, regions, 2]
                ...
                optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), config['training_params'].get('clip_grad_norm', 5.0))
                optimizer.step()
        ...
        for epoch in tqdm(range(n_epochs), desc="Training MR-GNODE"):
            ...
            for i in range(0, len(train_data), batch_size):
                ...
                batch_inputs = torch.stack([torch.tensor(x[0], dtype=torch.float32) for x in batch])
                batch_targets = torch.stack([torch.tensor(x[1], dtype=torch.float32) for x in batch])
...
def main():
    ...
    if config['model_type'] == 'motor_rnn':
        ...
        model = create_model(config)
        model, metadata = train_mr_gnode(model, config)
        save_model(model, config['cache_path'], metadata)
